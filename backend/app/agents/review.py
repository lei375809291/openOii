...
def _fallback_start_agent(feedback_type: str | None) -> str:
    if feedback_type in ("character", "shot", "render"):
        return "render"
...
def _is_retry_compose_feedback(feedback: str) -> bool:
    normalized = feedback.strip().lower()
    return any(keyword in normalized for keyword in _RETRY_COMPOSE_KEYWORDS)
...
class ReviewAgent(BaseAgent):
    ...
    async def run(self, ctx: AgentContext) -> Any:
        ...
        if not feedback:
            ...
            return {"start_agent": "plan", "reason": "未提供具体反馈"}

        retry_compose_requested = _is_retry_compose_feedback(feedback)
        ...
        if retry_compose_requested:
            start_agent = "compose"
            mode = "incremental"
            reason = reason or "检测到最终拼接重试请求，直接路由到视频合成"

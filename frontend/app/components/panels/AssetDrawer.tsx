		...
		<div className="card card-compact bg-base-200 border-2 border-base-content/10 hover:border-primary/40 transition-colors">
			...
			<div className="card-body p-2 gap-1">
				<div className="flex items-center gap-1">
					...
					<span className={`badge badge-xs ${typeCls[asset.asset_type] || "badge-ghost"}`}>
						{typeLabel[asset.asset_type] || asset.asset_type}
					</span>
				</div>
				{asset.description && (
					<p className="text-xs text-base-content/50 line-clamp-2">{asset.description}</p>
				)}
				<Button
					variant="ghost"
					size="sm"
			...
			>
				<div className="flex items-center justify-between p-3 border-b-2 border-base-content/10">
					<div className="flex items-center gap-1.5">
						...
						<span className="badge badge-xs badge-ghost">{data?.total ?? 0}</span>
					</div>
				...
				<div className="flex gap-1 px-3 py-2 border-b border-base-content/5">
					{["", "character", "scene", "style"].map((t) => {
						const label: Record<string, string> = { "": "全部", character: "角色", scene: "场景", style: "风格" };
						return (
							<button
								key={t}
								type="button"
				...
				<div className="p-3 overflow-y-auto" style={{ height: "calc(100vh - 100px)" }}>
					{items.length === 0 ? (
						...
						<div className="grid grid-cols-2 gap-2">
							{items.map((a) => (
								<AssetCard key={a.id} asset={a} onDelete={(id) => deleteMutation.mutate(id)} />
							))}
						</div>
					)}
				</div>

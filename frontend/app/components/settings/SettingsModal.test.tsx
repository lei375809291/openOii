...
describe('SettingsModal', () => {
  ...
  it('视频配置会随提供商切换', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByDisplayValue('https://video-openai.example.com')).toBeInTheDocument();
    });
  ...
  it('支持 text_provider 小写字段作为回退来源', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByDisplayValue('text-openai-fallback-key')).toBeInTheDocument();
    });
  });
  ...
  it('切换标签页并保持输入值', async () => {
    ...
    expect(screen.getByDisplayValue('redis://modified:6379/1')).toBeInTheDocument();
  });
  ...
  it('测试连接失败后显示错误提示', async () => {
    const user = userEvent.setup();
    vi.mocked(configApi.testConnection).mockRejectedValue(new Error('connection timeout'));
    ...
    await waitFor(() => {
      expect(configApi.testConnection).toHaveBeenCalledWith('llm', expect.any(Object));
  ...
  it('测试连接降级后显示警告提示', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('stream=false, generate=true')).toBeInTheDocument();
    });
  });
  ...
  it('文本配置区域会根据提供商切换展示', async () => {
    const user = userEvent.setup();
    const textConfig = [
      ...mockConfigData.filter(item => !['TEXT_PROVIDER', 'TEXT_BASE_URL', 'TEXT_API_KEY'].includes(item.key)),
      {
        key: 'TEXT_PROVIDER',
        value: 'anthropic',
        is_sensitive: false,
    ...
    await waitFor(() => {
      expect(screen.getByText('OpenAI 兼容接口配置')).toBeInTheDocument();
      const openaiSection = screen.getByText('OpenAI 兼容接口配置').closest('div');
      expect(openaiSection).toBeTruthy();
      if (openaiSection) {
        expect(within(openaiSection).getByDisplayValue('https://api.openai.com')).toBeInTheDocument();
      }
    });

    const anthropicRadio = screen.getByRole('radio', { name: /Anthropic Claude/ });
  ...
  it('显示真实值失败时给出错误提示', async () => {
    ...
    vi.mocked(configApi.revealValue).mockRejectedValue(new Error('reveal fail') as never);
